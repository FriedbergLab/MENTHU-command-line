...
pamScan <- function(pamList, cutDistList, ohList, targetList, exonList, exonStarts = NULL, findCut = FALSE, type = NULL, wiggle = TRUE, wiggleRoom = 39) {
	...
	if(class(matchesPos) == "list" && length(pamList) == 1){
		...
		pamFramePos <- data.frame(Target           = rep(pamList,        length(matchesPos)),
															...
															Sites            = matchesPos,
															MatchLength      = rep(nchar(as.character(pamList)), length(matchesPos)),
		...
		pamFrameNeg <- data.frame(Target           = rep(pamList,        length(matchesNeg)),
															...
															Sites            = matchesNeg + 1,
															MatchLength      = rep(nchar(as.character(pamList)), length(matchesNeg)),
...
talPal <- function(targetList, findCut = TRUE, wiggle = TRUE, wiggleRoom = 39,
									 tL = 15, tR = 18, spa = 14, exonList, exonStarts = NULL, Exon_Num = NULL) {
	require(stringr)
	require(plyr)
	...
	if (range == TRUE) {
		...
		patternLists <- getTalPatterns(tL, tR, spa) 
...
getTalPatterns <- function(tL, tR, spa){
	patListPos <- NULL
	patListNeg <- NULL
	patList    <- NULL
